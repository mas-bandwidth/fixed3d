...
B3_API b3Fixed b3GetLengthUnitsPerMeter( void );

/// Set the threshold for logging stalls.
B3_API void b3SetStallThreshold( b3Fixed seconds );

/// Get the threshold for logging stalls.
B3_API b3Fixed b3GetStallThreshold( void );

// Used to detect bad values. Fixed-point coordinates keep uniform precision
// everywhere, but squared distances must stay inside the Q48.16 range, so 100km
// remains the sanity limit.
#define B3_HUGE ( b3FixMul( B3_FIX( 1.0e5f ), b3GetLengthUnitsPerMeter() ) )
...
/// configurations.
#define B3_OVERLAP_SLOP ( b3FixMul( B3_FIX( 0.1f ), B3_LINEAR_SLOP ) )
...
#define B3_MESH_REST_OFFSET ( B3_LINEAR_SLOP )

/// The default contact recycling distance.
#define B3_CONTACT_RECYCLE_DISTANCE ( 10 * B3_LINEAR_SLOP )
...
#define B3_CONTACT_RECYCLE_ANGULAR_DISTANCE B3_FIX( 0.99240388f )

/// This is used to fatten AABBs in the dynamic tree. This allows proxies
/// to move by a small amount without triggering a tree adjustment. This is in meters.
/// @warning modifying this can have a significant impact on performance
#define B3_MAX_AABB_MARGIN ( b3FixMul( B3_FIX( 0.05f ), b3GetLengthUnitsPerMeter() ) )
...
#define B3_AABB_MARGIN_FRACTION B3_FIX( 0.125f )
