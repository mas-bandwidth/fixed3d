...
static StackResult RunStack( b3Fixed baseX )
{
	b3Pos base = (b3Pos){ baseX, B3_FIX( 0.0f ), B3_FIX( 0.0f ) };
	...
	b3BoxHull groundBox = b3MakeBoxHull( B3_FIX( 10.0f ), B3_FIX( 1.0f ), B3_FIX( 10.0f ) );
	...
	for ( int i = 0; i < STACK_COUNT; ++i )
	{
		...
		bodyDef.position = b3OffsetPos( base, (b3Vec3){ B3_FIX( 0.0f ), B3_FIX( 2.0f ) + b3FixMul( B3_FIX( 1.05f ) , b3FixFromInt( i ) ), B3_FIX( 0.0f ) } );
...
static b3Fixed RunBullet( b3Fixed baseX )
{
	b3Pos base = (b3Pos){ baseX, B3_FIX( 0.0f ), B3_FIX( 0.0f ) };
	...
	wallDef.position = b3OffsetPos( base, (b3Vec3){ B3_FIX( 5.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) } );
	b3BodyId wallId = b3CreateBody( worldId, &wallDef );
	b3BoxHull wallBox = b3MakeBoxHull( B3_FIX( 0.05f ), B3_FIX( 5.0f ), B3_FIX( 5.0f ) );
	...
	bulletDef.gravityScale = B3_FIX( 0.0f );
	bulletDef.position = base;
	bulletDef.linearVelocity = (b3Vec3){ B3_FIX( 200.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) };
	...
	bulletShapeDef.density = B3_FIX( 1.0f );
...
// still holds far from the origin where the swept query box rounds back to b3Fixed with large ULP.
...
{
	bool castHit;
	b3Fixed castRelX; // shape cast hit point x relative to the base
	bool overlapHit;
	b3Fixed moverFraction;
	int planeCount;
	bool rayHit;
	b3Fixed rayRelX; // world ray cast hit point x relative to the base
	bool shapeRayHit;
	b3Fixed shapeRayRelX; // direct shape ray cast hit point x relative to the base
} QueryResult;
...
static b3Fixed QueryCastCallback( b3ShapeId shapeId, b3Pos point, b3Vec3 normal, b3Fixed fraction, uint64_t materialId,
...
static QueryResult RunQueries( b3Fixed baseX )
{
	b3Pos base = (b3Pos){ baseX, B3_FIX( 0.0f ), B3_FIX( 0.0f ) };
	...
	b3World_Step( worldId, b3FixDiv( B3_FIX( 1.0f ) , B3_FIX( 60.0f ) ), 1 );

	QueryResult result = { 0 };

	// Sphere proxy swept from the left into the box, hitting the left face at relative x = -1
	b3Vec3 castPoint = { -B3_FIX( 5.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) };
	b3ShapeProxy castProxy = { &castPoint, 1, B3_FIX( 0.25f ) };
	CastContext castCtx = { base, false, B3_FIX( 0.0f ) };
	b3World_CastShape( worldId, base, &castProxy, (b3Vec3){ B3_FIX( 10.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) }, b3DefaultQueryFilter(), QueryCastCallback,
					   &castCtx );
	result.castHit = castCtx.hit;
	result.castRelX = castCtx.relX;

	// Sphere proxy sitting at the box center
	b3Vec3 overlapPoint = { B3_FIX( 0.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) };
	b3ShapeProxy overlapProxy = { &overlapPoint, 1, B3_FIX( 0.5f ) };
	...
	b3Capsule moverCast = { { -B3_FIX( 5.0f ), -B3_FIX( 0.3f ), B3_FIX( 0.0f ) }, { -B3_FIX( 5.0f ), B3_FIX( 0.3f ), B3_FIX( 0.0f ) }, B3_FIX( 0.25f ) };
	result.moverFraction =
		b3World_CastMover( worldId, base, &moverCast, (b3Vec3){ B3_FIX( 10.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) }, b3DefaultQueryFilter(), NULL, NULL );

	// Capsule overlapping the left face, should report a contact plane
	b3Capsule moverCollide = { { -B3_FIX( 1.1f ), -B3_FIX( 0.3f ), B3_FIX( 0.0f ) }, { -B3_FIX( 1.1f ), B3_FIX( 0.3f ), B3_FIX( 0.0f ) }, B3_FIX( 0.3f ) };
	int planeCount = 0;
	...
	b3Pos rayOrigin = b3OffsetPos( base, (b3Vec3){ -B3_FIX( 5.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) } );
	b3RayResult ray = b3World_CastRayClosest( worldId, rayOrigin, (b3Vec3){ B3_FIX( 10.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) }, b3DefaultQueryFilter() );
	result.rayHit = ray.hit;
	result.rayRelX = ray.hit ? b3SubPos( ray.point, base ).x : B3_FIX( 0.0f );

	// Direct shape ray cast against the same box
	b3WorldCastOutput shapeRay = b3Shape_RayCast( shapeId, rayOrigin, (b3Vec3){ B3_FIX( 10.0f ), B3_FIX( 0.0f ), B3_FIX( 0.0f ) } );
	result.shapeRayHit = shapeRay.hit;
	result.shapeRayRelX = shapeRay.hit ? b3SubPos( shapeRay.point, base ).x : B3_FIX( 0.0f );
...
{
	QueryResult origin = RunQueries( B3_FIX( 0.0f ) );
	ENSURE( origin.castHit );
	ENSURE( origin.overlapHit );
	ENSURE( origin.moverFraction < B3_FIX( 1.0f ) );
	ENSURE( origin.planeCount > 0 );
	ENSURE_SMALL( origin.castRelX + B3_FIX( 1.0f ), B3_FIX( 0.05f ) );
