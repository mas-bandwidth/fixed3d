...
// Mirrored geometry differs from its original only in the sign, which in two's-complement Q48.16
// flips the whole high word rather than one bit. A mixer that cannot carry high bits downward maps
// the whole family onto a few digests. (Upstream runs this over float sign bits; the property under
// test is the same and the byte pattern is the one this engine actually bakes.)
static int HashFixedSigns( void )
{
	enum
	{
		valueCount = 12,
		comboCount = 1 << valueCount
	};

	HashProbe probe = ProbeBegin( comboCount );

	b3Fixed values[valueCount];
	for ( int i = 0; i < valueCount; ++i )
	{
		values[i] = B3_FIX( 1.0f ) + B3_FIX( 0.25f ) * i;
	}

	for ( int mask = 0; mask < comboCount; ++mask )
	{
		b3Fixed flipped[valueCount];
		for ( int i = 0; i < valueCount; ++i )
		{
...
// Procedurally placed vertices land one quantum apart. Those blobs differ in a single low bit
// buried in a long run of identical bytes. One raw ULP of Q48.16 is B3_FIXED_EPSILON, so unlike
// the float case the perturbation is a plain add rather than a bit poke.
static int HashFixedUlp( void )
{
	enum
	{
		valueCount = 64
	...
	HashProbe probe = ProbeBegin( valueCount + 1 );

	b3Fixed values[valueCount];
	for ( int i = 0; i < valueCount; ++i )
	{
		values[i] = B3_FIX( 100.0f ) + B3_FIX( 1.0f ) * i;
	}
	...
	for ( int i = 0; i < valueCount; ++i )
	{
		b3Fixed saved = values[i];
		values[i] = saved + B3_FIXED_EPSILON;
	...
	HashResult result = ProbeEnd( &probe );
	ENSURE( result.overflow == false );
	ENSURE( result.count == valueCount + 1 );
	ENSURE( result.sawZero == false );
	ENSURE( result.collision == 0 );
	return 0;
}
...
static int HashBoxHulls( void )
{
	...
	for ( int i = 0; i < steps; ++i )
	{
		for ( int j = 0; j < steps; ++j )
		{
			...
			{
				b3BoxHull box =
					b3MakeBoxHull( B3_FIX( 0.5f ) + B3_FIX( 0.25f ) * i, B3_FIX( 0.5f ) + B3_FIX( 0.25f ) * j, B3_FIX( 0.5f ) + B3_FIX( 0.25f ) * k );
...
static int HashTransformedBoxHulls( void )
{
	...
	for ( int i = 0; i < steps; ++i )
	{
		for ( int j = 0; j < steps; ++j )
		{
			b3Vec3 axis = b3Normalize( (b3Vec3){ B3_FIX( 1.0f ), B3_FIX( 0.5f ) + B3_FIX( 0.1f ) * j, B3_FIX( 0.25f ) } );
			b3Transform transform = {
				.p = { B3_FIX( 0.125f ) * i, -B3_FIX( 0.25f ) * j, B3_FIX( 0.5f ) * ( i + j ) },
				.q = b3MakeQuatFromAxisAngle( axis, B3_FIX( 0.05f ) * ( i * steps + j ) ),
			};

			b3BoxHull box = b3MakeTransformedBoxHull( B3_FIX( 1.0f ), B3_FIX( 2.0f ), B3_FIX( 3.0f ), transform );
			ProbeAdd( &probe, box.base.hash, &box, box.base.byteCount );
		}
	}
...
static int HashProceduralHulls( void )
{
	...
	for ( int sides = 3; sides <= 18; ++sides )
	{
		for ( int r = 1; r <= 5; ++r )
		{
			for ( int h = 1; h <= 4; ++h )
			{
				b3HullData* cylinder = b3CreateCylinder( B3_FIX( 0.5f ) * h, B3_FIX( 0.25f ) * r, 0, sides );
				if ( cylinder != NULL )
				{
				...
				if ( sides >= 4 )
				{
					b3HullData* cone = b3CreateCone( B3_FIX( 0.5f ) * h, B3_FIX( 0.25f ) * r, B3_FIX( 0.1f ) * r, sides );
					if ( cone != NULL )
					{
...
static int HashHeightFields( void )
{
	...
	for ( int rows = 2; rows <= 14; ++rows )
	{
		for ( int cols = 2; cols <= 14; ++cols )
		{
			...
			b3HeightFieldData* wave = b3CreateWave( rows, cols, scale, B3_FIX( 0.25f ) * rows, B3_FIX( 0.125f ) * cols, false );
...
static int HashMeshes( void )
{
	...
	for ( int x = 2; x <= 10; ++x )
	{
		for ( int z = 2; z <= 10; ++z )
		{
			b3MeshData* grid = b3CreateGridMesh( x, z, B3_FIX( 1.0f ), 1, false );
			ProbeAdd( &probe, grid->hash, grid, grid->byteCount );
			b3DestroyMesh( grid );
		}
	}
	...
	for ( int radial = 3; radial <= 12; ++radial )
	{
		...
		{
			b3MeshData* torus = b3CreateTorusMesh( radial, tubular, B3_FIX( 1.0f ), B3_FIX( 0.25f ) );
...
static int HashVoxelDispersion( void )
{
	...
	{
		const b3Fixed cell = B3_FIX( 0.25f );
		b3Vec3 offset = {
			( i % 15 ) * cell,
			( ( i / 15 ) % 20 ) * cell,
			( i / 300 ) * cell,
		...
		b3BoxHull hull = b3MakeOffsetBoxHull( cell / 2, cell / 2, cell / 2, offset );
...
{
	b3BoxHull box1 = b3MakeBoxHull( B3_FIX( 1.0f ), B3_FIX( 2.0f ), B3_FIX( 3.0f ) );
	b3BoxHull box2 = b3MakeBoxHull( B3_FIX( 1.0f ), B3_FIX( 2.0f ), B3_FIX( 3.0f ) );
	ENSURE( box1.base.hash == box2.base.hash );

	b3HullData* cylinder1 = b3CreateCylinder( B3_FIX( 1.0f ), B3_FIX( 0.5f ), 0, 12 );
	b3HullData* cylinder2 = b3CreateCylinder( B3_FIX( 1.0f ), B3_FIX( 0.5f ), 0, 12 );
...
int HashTest( void )
{
	...
	RUN_SUBTEST( HashFixedSigns );
	RUN_SUBTEST( HashFixedUlp );
