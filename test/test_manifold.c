...
static int RidgeCrossingTest( void )
{
	...
	b3Fixed crossingAngles[] = { B3_FIX( 0.02f ), B3_FIX( 0.1f ), B3_FIX( 0.5f ) };
	...
	for ( int i = 0; i < ARRAY_COUNT( shallowAngles ); ++i )
	{
		...
		ENSURE_SMALL( manifold.normal.x, B3_FIX( 1e-4f ) );
		ENSURE_SMALL( manifold.normal.y - kHalfRoot2, B3_FIX( 1e-4f ) );
		ENSURE_SMALL( b3FixAbs( manifold.normal.z ) - kHalfRoot2, B3_FIX( 1e-4f ) );

		b3Fixed minSeparation = MinSeparation( &manifold );
		ENSURE( minSeparation < B3_FIX( 0.0f ) );
		ENSURE( minSeparation > b3FixMul( -B3_FIX( 2.0f ) , overlap ) );
	}
