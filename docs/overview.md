...
This is the manual for **Fixed3D**, a fork of Erin Catto's
[Box3D](https://github.com/erincatto/box3d) with the entire simulation
converted to Q48.16 fixed-point math (the `b3Fixed` type). The API keeps
Box3D's `b3` prefix by design, so existing Box3D code ports with minimal
changes, and behavior matches Box3D except where fixed point is called
...
Fixed3D also provides many collision routines that can be used even when
...
Fixed3D is written in portable C17. Most of the types defined in the
engine begin with the `b3` prefix, kept from Box3D so upstream code ports
unchanged. Hopefully this is sufficient to avoid name clashing with your
application.
...
Since Fixed3D is written in C, you are expected to be experienced in C
programming. Fixed3D should not be your first C programming project. You
should be comfortable with compiling, linking, and debugging.

> **Caution**:
> Fixed3D should not be your first C project. Please learn C
> programming, compiling, linking, and debugging before working with
> Fixed3D. There are many resources for this online.

## Scope

This manual covers the majority of the Fixed3D API. However, not every
aspect is covered. Please look at the Reference section and samples
application included with Fixed3D to learn more.

This manual is only updated with new releases. The latest version of
Fixed3D may be out of sync with this manual.
...
constraints; they are created automatically by Fixed3D.

### joint constraint
This is a constraint used to hold two or more bodies together. Fixed3D
...
### joint spring
A joint spring has a stiffness and damping. In Fixed3D spring stiffness is
...
that interact together. Fixed3D supports the creation of multiple worlds which
are completely independent.

### solver
The physics world has a solver that is used to advance time and to
resolve contact and joint constraints. The Fixed3D solver is a high
...
Fixed3D contains specialized algorithms to deal with tunneling. First, the
...
Fixed3D's primary purpose is to provide rigid body simulation. However,
...
Fixed3D works internally.

## Units

Fixed3D works with Q48.16 fixed-point numbers and tolerances have to be used
to make Fixed3D perform well. These tolerances have been tuned to work
well with meters-kilogram-second (MKS) units. In particular, Fixed3D has
...
> **Caution**:
> Fixed3D is tuned for MKS units. Keep the size of moving objects larger than 1cm.
...
> **Caution**:
> Fixed3D has uniform precision everywhere, but keep world coordinates
> within about ±10,000 km so squared-distance math stays representable —
> see [Large Worlds](large_worlds.md).

Fixed3D uses radians for angles. Orientation is stored as a quaternion
(`b3Quat`), so there is no concept of a body angle clamped to a range;
the full 3D rotation is tracked continuously.

> **Caution**:
> Fixed3D uses radians, not degrees.

## Changing the length units

Advanced users may change the length unit by calling `b3SetLengthUnitsPerMeter()`
at application startup. If you keep Fixed3D in a shared library, you will need
to call this if the shared library is reloaded.

It is harder to get support for using Fixed3D if you change the unit
...
Fast memory management plays a central role in the design of the Fixed3D
...
These definitions contain all the information needed to build the Fixed3D object. By using
...
ids as opaque data, however you may zero initialize all Fixed3D ids and they will be considered
*null*.
...
For convenience, Fixed3D will destroy all shapes on a body when the body is destroyed. You don't need to store the shape id.
