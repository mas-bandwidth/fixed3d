...
void b3CollideTriangleAndSphere( b3LocalManifold* manifold, int capacity, const b3Vec3* triangleA, const b3Sphere* sphereB )
{
	...
	b3Fixed maxDistance = radius + speculativeDistance;
	...
	// p = 0.5 * (c + q) - 0.5 * r * n
	b3Vec3 contactPoint = b3MulSV( B3_FIX( 0.5f ), b3Add( b3Sub( center, b3MulSV( radius, normal ) ), closest.point ) );
								  ...
								  b3SimplexCache* cache )
{
	...
	b3Vec3 v1 = triangleA[0], v2 = triangleA[1], v3 = triangleA[2];
	b3Plane plane = b3MakePlaneFromPoints( v1, v2, v3 );
	b3Vec3 capsuleCenter = b3Lerp( capsuleB->center1, capsuleB->center2, B3_FIX( 0.5f ) );
	...
	distanceInput.proxyA = (b3ShapeProxy){ triangleA, 3, B3_FIX( 0.0f ) };
							   ...
							   const b3HullData* hullB, b3SATCache* cache, bool enableSpeculative )
{
	...
	b3Fixed offset = b3PlaneSeparation( trianglePlane, hullB->center );
	...
	if ( manifold->pointCount == 0 )
	{
		...
		input.proxyB = (b3ShapeProxy){ .points = hullPoints, .count = hullB->vertexCount, .radius = B3_FIX( 0.0f ) };
