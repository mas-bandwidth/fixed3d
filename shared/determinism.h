...
typedef struct WavePileData
{
	...
	struct b3HeightFieldData* heightField;
	int stepCount;
	int sleepStep;
	uint32_t hash;
} WavePileData;
...
// query results. Any query divergence cascades into the final state. Content is authored
// local and offset onto GetSceneOrigin at the world boundary per the scene-origin invariant.
#define QUERY_SPAWN_COUNT 50
#define QUERY_SPAWN_CAST_RADIUS B3_FIX( 0.5f )
