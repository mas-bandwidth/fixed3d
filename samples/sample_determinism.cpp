...
public:
		...
		: Sample( context )
	{
		if ( context->restart == false )
		{
			m_camera->SetView( 45.0f, 25.0f, 30.0f, SampleOrigin() );
		}
	...
	void Render() override
	{
		...
		Vec4 castColor = m_data.castFraction < B3_FIX( 1.0f ) ? MakeColor( b3_colorCyan ) : MakeColor( b3_colorGray );
...
public:
		...
		: Sample( context )
	{
		...
		// The sample runs the scene at the shared origin; the unit test pins its goldens
		// at b3Pos_zero, so the on-screen hash differs from the pinned constants.
